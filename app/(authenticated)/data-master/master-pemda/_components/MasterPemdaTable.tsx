...
export default function MasterPemdaTable({
  ...
  // --- 1. HOOKS (Harus selalu di paling atas) ---
  ...
  // --- 3. EARLY RETURN UNTUK LOADING ---
  // Diletakkan setelah hooks agar urutan hook tetap konsisten
  if (loading) {
    return <HybridLoader />
  }
    ...
    <div className="space-y-6">
      {data.length === 0 ? (
        <div className="bg-white rounded-xl p-10 text-center border border-dashed">
          <p className="text-gray-400 text-sm">Tidak ada data pemda ditemukan.</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {paginatedData.map((item) => (
              ...
              >
                ...
                <div className="w-20 h-20 rounded-full overflow-hidden bg-gray-50 flex items-center justify-center mb-3 shrink-0 border border-gray-100">
                  {item.logo ? (
                    <img
                      src={item.logo}
                      alt={item.name}
                    ...
                    <Building2 className="size-9 text-gray-300" />
                  )}
                </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tindakan ini tidak dapat dibatalkan. Data pemda akan dihapus secara permanen dari server.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
