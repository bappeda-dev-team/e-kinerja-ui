...
import * as React from "react" // Gunakan import * as React agar konsisten
...
export default function MasterPemdaClient() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Kasih delay dikit biar animasinya kerasa
  ...
  const handleAdd = async (item: { name: string; logo?: File }) => {
    try {
      ...
      if (res.status !== 200 && res.status !== 201) {
        throw new Error(res.message || "Gagal membuat data Pemda")
      }
  ...
  const handleEdit = async (updated: { id: string; name: string; logoFile?: File }) => {
    try {
      ...
      if (resUpdate.status !== 200) {
        throw new Error(resUpdate.message || "Gagal memperbarui nama Pemda")
      }
      if (updated.logoFile) {
        try {
  ...
  return (
    <div className="px-4">
      ...
      {/* GANTI LOADER LAMA DENGAN HYBRID LOADER */}
      {loading ? (
        <HybridLoader />
      ) : (
        <MasterPemdaTable
