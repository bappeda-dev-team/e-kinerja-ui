...
export function AttentionPanel({ items, loading }: Props) {
    ...
    <div className="lg:col-span-2 xl:col-span-3 rounded-2xl border border-gray-200 bg-white shadow-sm flex flex-col">
      <div className="border-b border-gray-200 px-6 py-5">
        <div className="flex items-center gap-2 text-gray-900">
          ...
          <h2 className="text-lg font-bold">Perlu Perhatian Segera</h2>
        </div>
        <p className="mt-1 text-sm text-gray-500">Laporan yang membutuhkan aksi (revisi atau mendekati deadline).</p>
      </div>
      ...
      <div className="flex-1 p-4">
        {loading ? (
          ...
          <EmptyState label="Hore! Tidak ada laporan yang perlu perhatian segera." />
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`mt-0.5 h-2.5 w-2.5 shrink-0 rounded-full ${isRevisi ? "bg-red-500" : "bg-amber-500"}`} />
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="font-semibold text-gray-900">{item.pemda}</h3>
                        <span className="rounded-md bg-gray-200/50 px-2 py-0.5 text-[10px] font-bold text-gray-600">
                          {item.kategori}
                        </span>
                      </div>
                      <p className="mt-1 text-sm text-gray-600 line-clamp-1">
                        <span className="font-semibold mr-1">{isRevisi ? "Revisi:" : "Menunggu:"}</span>
                        {item.alasan}
                      </p>
                    </div>
                  </div>
