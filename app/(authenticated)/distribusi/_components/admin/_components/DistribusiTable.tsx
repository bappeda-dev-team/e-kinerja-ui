...
import { getStatusMeta, formatTgl } from "@/app/(authenticated)/distribusi/_components/super-admin/_components/DistribusiUtils"
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-[6px_6px_54px_rgba(0,0,0,0.05)] overflow-hidden">
      <Table>
        ...
        <TableBody>
          {paginatedRows.length === 0 ? (
            <TableRow>
              <TableCell colSpan={9} className="text-center py-12 text-sm text-[#202224]/40">
            ...
            return (
              ...
              >
                ...
                <TableCell className="px-4 py-3 text-xs text-[#797A7C]">
                  <button
                    onClick={(e) => { e.stopPropagation(); onShowKomentar(row) }}
      ...
      <div className="flex flex-col gap-3 border-t border-gray-200 px-4 py-4 md:flex-row md:items-center md:justify-between">
        <p className="text-sm text-[#202224]/60">
          Menampilkan {allRows.length === 0 ? "0-0" : `${(currentPage - 1) * rowsPerPage + 1}-${Math.min(currentPage * rowsPerPage, allRows.length)}`} dari {allRows.length} data
        </p>
