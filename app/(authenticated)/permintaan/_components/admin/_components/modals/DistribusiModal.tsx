...
import { getUsers } from "@/services/distribusi.service"
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        toast.error("Gagal memuat daftar programmer")
      } finally {
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-md bg-white p-0 overflow-hidden border-[#B9B9B9] rounded-2xl">
        <DialogHeader className="px-6 py-4 border-b border-[#E0E0E0]">
          <DialogTitle className="text-xl font-bold text-[#202224]">Distribusi Pekerjaan</DialogTitle>
        </DialogHeader>
